...
echo "==> enable Secure cookies for demo tenant"
ENV=/opt/rsgzgl/tenants/demo/app.env
...
if [ -f /opt/rsgzgl/tenants/demo/credentials.txt ]; then
  sed -i "s|^URL=.*|URL=https://${DOMAIN}|" /opt/rsgzgl/tenants/demo/credentials.txt || true
fi
