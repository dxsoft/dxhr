...
final class LicenseCrypto {
    ...
    static String localPolicyToJson(LicenseLocalPolicy policy) {
        if (policy == null) {
            return "";
        }
        return localPolicyJson(policy);
    }

    static LicenseLocalPolicy parseLocalPolicyFromSeed(String seedJson) {
        return parseLocalPolicy(objectBlock(seedJson, "localPolicy"));
    }

    static LicenseLocalPolicy parseLocalPolicyJson(String policyJson) {
        if (policyJson == null || policyJson.isBlank()) {
            return null;
        }
        String block = policyJson.trim();
        if (!block.startsWith("{")) {
            block = "{" + block + "}";
        }
        return parseLocalPolicy(block);
    }
